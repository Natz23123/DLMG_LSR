...
def secvente_to_data():
    mp_hands = mp.solutions.hands
    hands = mp_hands.Hands(static_image_mode=True, max_num_hands=1)

    DATA_DIR = "dynamic_data"
    output_file = "data_secvente.json"
    ...
    for letter in os.listdir(DATA_DIR):
        ...
        for secvention_name in os.listdir(letter_path):
            secvention_path = os.path.join(letter_path, secvention_name)
            secvention_data=[]

            for iteration in os.listdir(secvention_path):
                iteration_path = os.path.join(secvention_path, iteration)
                img = cv.imread(iteration_path)
                if img is None:
                    continue
                ...
                frame_data = extract_data(lm)
                secvention_data.append(frame_data)

            if secvention_data:
                dataset.append({"class": letter, "data": secvention_data})

    with open(output_file, "w") as f:
        json.dump(dataset, f, indent=5)
